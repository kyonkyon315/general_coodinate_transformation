...
#include <cmath>
#include "none.h"
#include "advection_calculator.h"

using Value = double;
template<typename TargetFunction,typename Advections,typename Jacobian,typename Scheme,typename BoundaryCondition,typename Current>
...
private:
    ...
    Current& current;
    const AdvectionCalculator<TargetFunction,Advections,Jacobian> advection_calculator;
    ...
    // helper: index_sequence を最初の引数に置く
    ...
    void solve_leaf_impl_indices(Value dt, Value& Up, Value& Um, std::index_sequence<Is...>, Ints... indices){
        const Value nyu_p_half = - advection_calculator.template flux_p_half<Target_Dim,Ints...>(dt,indices...);
        const Value nyu_m_half = - advection_calculator.template flux_m_half<Target_Dim,Ints...>(dt,indices...);
...
public:
    AdvectionEquation(
        ...
        current(current),
        advection_calculator(target_func,advections,jacobian)
    {
